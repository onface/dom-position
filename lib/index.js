...
const domPos = function (options) {
    ...
    // set baseOn Offse
    positionLeft = getBaseOnOffset(positionLeft, options.baseOn.el[0], options.el.clientWidth + getBorderWidth(options.el).left + getBorderWidth(options.el).right, 'el')
    positionTop = getBaseOnOffset(positionTop, options.baseOn.el[1], options.el.clientHeight  + getBorderWidth(options.el).top + getBorderWidth(options.el).bottom, 'el')

    positionLeft = getBaseOnOffset(positionLeft, options.baseOn.target[0], options.target.clientWidth +  + getBorderWidth(options.target).left + getBorderWidth(options.target).right)
    positionTop = getBaseOnOffset(positionTop, options.baseOn.target[1], options.target.clientHeight + getBorderWidth(options.target).top + getBorderWidth(options.target).bottom)
